...
def finalizeComponent(drvMacTester):
    res = Database.activateComponents(["tcc2","pic32cx_bz2_devsupport"])
...
def instantiateComponent(drvMacTester):
    print('MAC Tester Component')
    ...
    global UseRTOS
    UseRTOS = drvMacTester.createBooleanSymbol("USE_FREERTOS", None)
...
#####################################################################################################
    # System Definition
    ptcSystemDefFile = drvMacTester.createFileSymbol("ZIGBEE_SYS_DEF", None)
    ptcSystemDefFile.setType("STRING")
    ptcSystemDefFile.setOutputName("core.LIST_SYSTEM_DEFINITIONS_H_INCLUDES")
    ptcSystemDefFile.setSourcePath("driver/zigbee/application/zigbee_only/WSNTester/templates/system/definitions.h.ftl")
    ptcSystemDefFile.setMarkup(True)

    # Custom Antenna Gain Enabled
    customAntennaSpecified = drvMacTester.createBooleanSymbol('USE_CUSTOM_ANT_GAIN', None)
    ...
    customAntennaGain = drvMacTester.createIntegerSymbol('CUSTOM_ANT_GAIN', None)
    ...
    customAntennaRegion = drvMacTester.createStringSymbol('CUSTOM_ANT_REGION', None)
    ...
    zigbeeFreeRTOSConfig = drvMacTester.createFileSymbol("ZIGBEE_RTOS_CONFIG_HEADER", None)
    zigbeeFreeRTOSConfig.setSourcePath("/driver/zigbee/application/zigbee_only/Zigbee_ZgpSensor_Application/configs/FreeRTOSConfig.h.ftl")
    zigbeeFreeRTOSConfig.setOutputName("FreeRTOSConfig.h")
    zigbeeFreeRTOSConfig.setDestPath("")
    zigbeeFreeRTOSConfig.setProjectPath("config/" + configName)
    zigbeeFreeRTOSConfig.setType("HEADER")
    zigbeeFreeRTOSConfig.setOverwrite(True)
    zigbeeFreeRTOSConfig.setMarkup(True)
    ...
    for i, f, p in zigbeeInitTemplates:
        zigbeeInit.append( drvMacTester.createFileSymbol(i, None))
...
################################### HEADER FILES ####################################################
    ...
    wsnTesterIncFiles = [
        ['application/zigbee_only/WSNTester/configs/bitcloudPreInclude.h',                        condAlways],
        ['application/zigbee_only/WSNTester/configs/stackConfig.h',                               condAlways],
        ['application/zigbee_only/WSNTester/configs/zigbeeAppConfig.h',                           condAlways],
        ['application/zigbee_only/WSNTester/configs/Mac2_CommonMac_StdzgpSec_Pic32cx_Rf233_Gcc_Makerules.h',      condAlways],

        ['application/zigbee_only/WSNTester/MACTesterApp/include/assert.h',                       condAlways],
        # ['application/zigbee_only/WSNTester/MACTesterApp/include/framework_defs.h',               condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/include/mac_command.h',                  condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/include/mac_parser.h',                   condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/include/mac_parser_fxns.h',              condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/include/mac_proc.h',                     condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/include/mac_proc_fxns.h',                condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/include/mac_server.h',                   condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/include/mac_server_fxns.h',              condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/include/mac_wrapper.h',                  condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/include/os.h',                           condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/include/zgb_task.h',                     condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/include/zgb_api.h',                     condAlways],
    ]
    ...
    for incFileEntry in wsnTesterIncFiles:
        importIncFileNonStack(drvMacTester, configName, incFileEntry)
...
################################### APS FILES #######################################################
    ...
    zdrvStackSYSENVSrcFiles = [
        ['systemenvironment/src/dbg.c',                 condMacSource],
    ...
    applicationCommonSrcFiles = [
        ['application/zigbee_only/WSNTester/common/src/zigbee_init.c',                                condAlways],
        # ['application/zigbee_only/common_files/src/errH.c',                                         condAlways],
        ['application/zigbee_only/WSNTester/common/src/zgb_task.c',                                   condAlways],
        # ['application/zigbee_only/WSNTester/MACTesterApp/src/initialization.c',                     condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/src/mac_command.c',                          condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/src/mac_parser.c',                           condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/src/mac_proc.c',                             condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/src/mac_server.c',                           condAlways],
        ['application/zigbee_only/WSNTester/MACTesterApp/src/mac_tester.c',                           condAlways],

    ]


    for srcFileEntry in applicationCommonSrcFiles:
        importSrcFileNonStack(drvMacTester, configName, srcFileEntry)
    ...
    zigbeeStackdrvIncPaths = [
        ...
        ['/zigbee/WSNTester/MACTesterApp/configs',                                     condAlways],
        ['/zigbee/WSNTester/MACTesterApp/include',                                     condAlways],
    ...
    app_zgbHeaderFile = drvMacTester.createFileSymbol(None, None)
    ...
    app_SourceFile = drvMacTester.createFileSymbol(None, None)
    app_SourceFile.setSourcePath('driver/zigbee/application/zigbee_only/WSNTester/templates/app.c')
    ...
    preprocessormac = drvMacTester.createSettingSymbol('ZIGBEE_PREPRECESSOR', None) 
    preprocessormac.setValue('_PIC32CX_;PIC32CX_CHIP_SOC;MAC_TEST_APP')
    ...
    preprocessormac = drvMacTester.createSettingSymbol('ZIGBEE_ISOLATEFUNCTION', None) 
    ...
    preprocessormac = drvMacTester.createSettingSymbol('ZIGBEE_PLACEDATASECTION', None) 
    preprocessormac.setValue('false')
    preprocessormac.setCategory('C32')
    ...
    warningerr = drvMacTester.createSettingSymbol('WARNINGG_ERR', None)
    warningerr.setValue('false')
    warningerr.setCategory('C32')
    warningerr.setKey('make-warnings-into-errors')

    warningadd = drvMacTester.createSettingSymbol('WARNINGG_ADD', None)
    warningadd.setValue('false')
    warningadd.setCategory('C32')
    warningadd.setKey('additional-warnings')

    optlevel = drvMacTester.createSettingSymbol('PET_LEVEL', None)
    optlevel.setValue('-O2')
    optlevel.setCategory('C32')
    optlevel.setKey('optimization-level')

################################### Library File ####################################################
    setAdditionaloptionXC32GCC = drvMacTester.createSettingSymbol('SET_XC32_GCC_ADD_INC', None)
    setAdditionaloptionXC32GCC.setValue('-include"WSNTester/configs/zigbeeAppConfig.h"')
    ...
    setAdditionaloptionXC32GCC1.setValue('-include"WSNTester/configs/Mac2_CommonMac_StdzgpSec_Pic32cx_Rf233_Gcc_Makerules.h"')
...
def importIncFile(component, configName, incFileEntry, firmwarePath = None):
    ...
    incFileSym.setDestPath('/zigbee/lib/include/' + secDName + '')
    incFileSym.setProjectPath('config/' + configName + '/zigbee/lib/include/'+ secDName + '')
...
def importIncFileNonStack(component, configName, incFileEntry, firmwarePath = None):
    ...
    secTname = secDName.replace('Zigbee_Device_Application','zigbee_device')
    secDName = secTname.replace('common_files','common_files')
    secDName = secTname.replace('zgb_wrappers','zigbee_af_interface')
...
def importSrcFile(component, configName, srcFileEntry, firmwarePath = None):
    ...
    srcFileSym.setOutputName(srcFile.rsplit('/', 1)[-1])
    srcFileSym.setDestPath('/zigbee/zigbee_systemresource/' + secDName)
    srcFileSym.setProjectPath('config/' + configName + '/zigbee/zigbee_systemresource/' + secDName)
    srcFileSym.setType('SOURCE')
    srcFileSym.setEnabled(isEnabled)
...
def setIncPath(component, configName, incPathEntry):
    ...
    incPath = incPath.replace('/Components','/zigbee/lib/include')
...
def onAttachmentConnected(source, target):
    ...
    if (connectID == "Zigbee_USART"):
        ...
        Database.setSymbolValue("lib_wolfcrypt", "wolfcrypt_aes_cbc", False)
