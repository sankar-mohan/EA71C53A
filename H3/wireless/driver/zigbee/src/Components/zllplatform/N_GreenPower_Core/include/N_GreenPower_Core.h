...
/** Subscribes for N_GreenPower_Core callbacks.
    \param pCallback The callback from subscriber
*/
void N_GreenPower_Core_Subscribe(const N_GreenPower_Core_Callback_t* pCallback);

/** Send a ZCL command as if it came from a ZigBee GreenPower Device.
    \param aliasSourceAddress The aliased source address.
    \param aliasSequenceNumber The network sequence number to fake (use securityFrameCounter from the callback).
    \param pRequest Pointer to the ZCL command to send.
    \param pdestinationAddress The address to send the command to. Only groupcast and broadcast are currently supported.
    \param forwardingRadius number of hops the alias frame is allowed to travel through the network.
...
/** Authenticate the ZigBee GreenPower message
    \param messageLength The length of the message in *message.
    \param message The message.
    \param micLength The length of the mic in *mic.
    \param mic The mic.
    \param key The security key to authenticate the mic with.
    \param nonce The nonce to authenticate the mic with. See Green Power Spec A.1.5.4.3.1.
    \param done The callback with the result to be called when the message is authenticated.
*/
...
/** Decrypt and authenticate the ZigBee GreenPower security key.
    \param gpdSourceId The GPD source ID.
    \param encryptedKey The encrypted security key that needs to be decrypted.
    \param key The key to decrypt the encryptedKey.
    \param nonce The nonce used for decrypting the security key. See Green Power Spec A.1.5.4.3.1.
    \param mic The mic used for authenticating the decrypted security key.
    \param done The callback with the result to be called when the key is decrypted and authenticated.
*/
...
/** Send a device announce for a GreenPower device.
    \param aliasSourceAddress The aliased source address.
    \param securityCapable TRUE if the device is security capable, otherwise FALSE.
    \return Status of the sent message.
*/
