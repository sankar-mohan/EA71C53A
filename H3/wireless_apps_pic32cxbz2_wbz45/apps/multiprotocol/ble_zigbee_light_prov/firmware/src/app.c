...
static void APP_Init(void)
{
    ...
    appSnprintf("Combo Light Application Version: ");
    appSnprintf(COMBO_APP_VERSION);
        ...
        case APP_STATE_SERVICE_TASKS:
        {
            if (OSAL_QUEUE_Receive(&appData.appQueue, &appMsg, OSAL_WAIT_FOREVER))
            {
                switch (p_appMsg->msgId)
                {
                    ...
                    case APP_MSG_TRS_BLE_ADV_CTRL:
                    {
                        APP_BLE_Adv_TimerHandler();
                    }
                    break;
                    
                    case APP_MSG_TRS_BLE_SENSOR_READ:
                    {
                    ...
                    case APP_MSG_BUTTON_INT:
                    {
                        APP_Button_Handler((uint8_t*)(p_appMsg->msgData)); 
                    }
                    break;
                    
                    case APP_MSG_ZB_STACK_EVT:
                    {
